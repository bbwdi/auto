...
class InventoryManageDeviceListPage(BasePage):
    ...
    def input_device_check_search_info(self,check_list):
        ...
        if check_list[1]!=u'':
            self.input_star_and_end_time(check_list[1])
        if check_list[2]!=u'':
...
class InventoryManageFacilityListPage(InventoryManageDeviceListPage):
    '''库存管理-设施列表页面'''
    ...
    def input_facility_check_search_info(self,check_list):
        '''输入查看部分的查询信息'''
        if check_list[0]!=u'':
            self.input_star_and_end_time(check_list[0])
        if check_list[1]!=u'':
            self.input_text_message_for_inside_text(u'请输入仓库名称',check_list[1])
        self.click_button_for_one(u'查询')
        sleep(1)

class InventoryManageConsumableListPage(InventoryManageDeviceListPage):
    '''库存管理-耗材列表页面'''
...
class InventoryManageFittingListPage(InventoryManageDeviceListPage):
    '''库存管理-配件列表页面'''

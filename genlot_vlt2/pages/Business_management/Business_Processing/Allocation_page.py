...
serialnum='''gavin'''
clickModel1='''(//*[@id="pane-1"]//input[@placeholder='请选择'])[2]'''#点击第一行型号
...
num_path='''(//*[@id="pane-1"]//input[@placeholder='请输入数量'])[1]'''



class Allocation(business_process):
    def title(self):
        '''申请标题 '''
        self.input_xpath(title_path,titlename)
        return  titlename

    def Ex_warehouse(self):
        '''出库仓库'''
        time.sleep(1)
        self.get_element_by_xpath(warehouse_path1).click()
        time.sleep(0.5)
        tt=self.get_elements_by_xpath(click_path)
        tt1=len(tt)/2
        kk=self.randomnum(tt1)
        click_path1=self.xparh_join(click_path,tt1+kk)

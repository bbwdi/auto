...
titlename=u'''autotest资源采购'''
name='''//*[@id="pane-1"]//input[@placeholder='请输入采购员名称']'''
user_name='''//*[@id="app"]//div//span/span[@class='user-name']'''
date='''//*[@id="pane-1"]//input[@placeholder='请选择到货日期']'''
now_path='''//table[1]//td[@class='available today']'''
...
reason_path='''(//*[@id="pane-1"]//textarea[@placeholder='请输入采购说明'])'''
...
remake=u'''备注信息year'''
# money='''(//*[@id="pane-1"]//tr[@class='el-table__row']/td[7]/div)[3]'''
money='''//*[@id="pane-1"]/div[2]/div[2]/div/div[3]/form/div/div[3]/table/tbody/tr/td[7]/div'''



class Procure(business_process):
    def title(self):
        '''申请标题 '''
    ...
    def name(self):
        '''采购员'''
        tt=self.get_element_by_xpath(user_name).text
        self.input_xpath(name,tt)
        return  str(tt)

    def deliverdate(self):
        '''到货时间默认为当前时刻'''
        time.sleep(1)
        self.get_element_by_xpath(date).click()
        time.sleep(0.5)
        self.get_element_by_xpath(now_path).click()
        kk=datetime.datetime.now()#获取当前时间，拿当前日期对比
        kk1=str(kk.date())
        kk2=kk1 +" 00:00:00"
        return str(kk2)
    ...
    def Serial(self):
        yy=str(self.randomnum(1000000))
        serialnum1=serialnum + yy
        self.input_xpath(serial,serialnum1)#序列号
        return  serialnum1
	...
	(SELECT  t.*  FROM  T_ACT_BUSINESS t WHERE t.title = 'autotest资源采购' 
